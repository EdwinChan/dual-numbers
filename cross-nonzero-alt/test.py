...
class DualExactTest(DualTest):
  ...
  def test_hash(self):
    for x, in self.sample():
      y = dual.Dual({**dict.fromkeys(range(max(x.keys()) + 1), 0), **x})
      z = dual.Dual({0: x[0]})
      super().assertEqual(hash(x), hash(y))
      super().assertEqual(hash(z), hash(z[0]))
...
class DualSymbolTest(DualExactTest):
  ...
  def setUpClass(cls):
    ...
    def make_dual(symbol):
      return dual.Dual(enumerate(sympy.symbols(f'{symbol}:{cls.term_count}')))
  ...
  def test_asin_log(self):
    for x, in self.sample():
      y = dual.asin(x)
      y[0] = y[0].subs(
        sympy.asin(x[0]), self.asin_to_log(sympy.sqrt, sympy.log, x[0]))
  ...
  def test_acos_log(self):
    for x, in self.sample():
      y = dual.acos(x)
      y[0] = y[0].subs(
        sympy.acos(x[0]), self.acos_to_log(sympy.sqrt, sympy.log, x[0]))
...
class DualNumberTest(DualTest):
  ...
  def setUpClass(cls):
    ...
    pures += [dual.Dual({0: cls.random()}) for _ in range(cls.pure_count)]
